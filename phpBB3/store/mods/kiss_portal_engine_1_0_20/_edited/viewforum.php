...
$template->assign_vars(array(
	...
	'FOLDER_NEWS_IMG'		=> $user->img('news_read', 'NEW_POST_NEWS'),
	'FOLDER_NEWS_NEW_IMG'	=> $user->img('news_unread', 'NO_NEW_POST_NEWS'),
));
...
if ($forum_data['forum_type'] == FORUM_POST)
{
	...
	$sql = $db->sql_build_query('SELECT', array(
		...
		'WHERE'		=> 't.forum_id IN (' . $forum_id . ', 0)
			AND t.topic_type IN (' . POST_ANNOUNCE . ', ' . POST_GLOBAL . ', ' . POST_NEWS . ', ' . POST_NEWS_GLOBAL . ')',
...
if (sizeof($topic_list))
{
	...
	foreach ($topic_list as $topic_id)
	{
		...
		$s_type_switch_test = ($row['topic_type'] == POST_ANNOUNCE || $row['topic_type'] == POST_GLOBAL || $row['topic_type'] == POST_NEWS || $row['topic_type'] == POST_NEWS_GLOBAL ) ? 1 : 0;
		...
		$s_type_switch = ($row['topic_type'] == POST_ANNOUNCE || $row['topic_type'] == POST_GLOBAL || $row['topic_type'] == POST_NEWS || $row['topic_type'] == POST_NEWS_GLOBAL) ? 1 : 0;
